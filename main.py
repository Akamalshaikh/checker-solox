...
# Bot configuration
TOKEN = "7917405601:AAHs36nL9Kfgj6R4GeFKuZ0os0O1-NELUno"  # Replace with your bot token
OWNER_ID = 6048265605  # Owner ID for @Its_solox
...
def start(message):
    ...
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘠𝘰𝘶 𝘢𝘳𝘦 𝘯𝘰𝘵 𝘢𝘱𝘱𝘳𝘰𝘷𝘦𝘥 𝘵𝘰 𝘶𝘴𝘦 𝘵𝘩𝘪𝘴 𝘣𝘰𝘵. 𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @Its_solox")
        return
...
def add_user(message):
    ...
    try:
        ...
        bot.reply_to(message, "𝗣𝗿𝗼𝘃𝗶𝗱𝗲 𝗮 𝘂𝘀𝗲𝗿 𝗜𝗗 𝘁𝗼 𝗮𝗽𝗽𝗿𝗼𝘃𝗲.")
...
def handle_document(message):
    ...
    if user_id in load_banned_users():
        ...
        return
    if user_id not in load_approved_users():
        bot.reply_to(message, "𝘊𝘰𝘯𝘵𝘢𝘤𝘵 𝘵𝘩𝘦 𝘰𝘸𝘯𝘦𝘳- @Its_solox")
        return
        ...
        with open(file_path, "r") as file:
            ...
            for cc in lines:
                if stop_processing.get(user_id, False):
                    bot.send_message(message.chat.id, "🛑 𝙋𝙧𝙤𝙘𝙚𝙨𝙨𝙞𝙣𝙜 𝙨𝙩𝙤𝙥𝙥𝙚𝙙 𝙗𝙮 𝙪𝙨𝙚𝙧.")
                    break
